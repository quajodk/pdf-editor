...
(async () => {
  console.log('Starting Session 53 verification...\n');
  ...
  try {
    ...
    const editTextButton = await page.evaluateHandle(() => {
      ...
      return buttons.find(btn => btn.textContent.includes('📝') || btn.getAttribute('title')?.includes('Edit Text'));
    });
    ...
    // 4. Test some other tools
    console.log('4. Testing toolbar tools...');

    // Test Text tool
    const textButton = await page.evaluateHandle(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      return buttons.find(btn => btn.textContent.includes('T') && btn.getAttribute('title')?.includes('Add Text'));
    });
    if (textButton) {
    ...
    const penButton = await page.evaluateHandle(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      return buttons.find(btn => btn.textContent.includes('✏️') || btn.getAttribute('title')?.includes('Pen'));
    });
    if (penButton) {
      await penButton.asElement().click();
      await new Promise(resolve => setTimeout(resolve, 500));
      console.log('   ✓ Pen tool working');
    }

    await page.screenshot({ path: 'session53-verify-04-toolbar.png' });
